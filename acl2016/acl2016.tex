...
While there has been a lot of work suggesting that linguistic accommodation is important, there are a number of open questions. One major one is how the desire to accommmodate is balanced against a need to move the conversation forward and introduce new words and concepts.  We propose that linguistic accommodation is modulated by the discourse act being performed, with certain acts more or less accommodating depnding on their purpose within the conversation.

Second, there is the question of different components of linguistic accommodation. While linguistic accommodation has been studied in multiple ways, the most prominent strand has focused on assessing accommodation in terms of word categories, with accommodation meaning that one speaker modulates their word category frequencies dependent on their interlocutor. There are conflicting expectations about how different types of accommodation should interact, ranging from the Interactive Alignment Model's expectation that different types of accommodation should be approximately uniform to arguments that, for instance, lexical and syntactic accommodation ought to push in different directions (Healey et al).

We attempt to drill into these questions, proposing a variant of an existing model of linguistic accommodation, and using it to separately estimate different types of accommodation.  We then apply this model to different datasets and different utterance types within a dataset, finding that accommodation is not monolithic but rather depends on a variety of conversational variables in interesting ways.
